...
const inMemoryHistory = new Map<string, City[]>();
...
class HistoryService {
  ...
  async addCity(sessionId: string, cityName: string): Promise<City | null> {
    ...
    if (!pool) {
      ...
      if (cities.find((c) => c.name.toLowerCase() === cityName.toLowerCase())) {
        console.log(
          `City "${cityName}" already exists in session ${sessionId}`
        );
        return null;
      }
    ...
    try {
      ...
      const result = await pool.query(
        `INSERT INTO search_history (id, session_id, city_name) 
         VALUES ($1, $2, $3) 
         ON CONFLICT (session_id, city_name) DO NOTHING
         RETURNING id, city_name`,
        [cityId, sessionId, cityName]
      );
  ...
  async getActiveSessionsCount(): Promise<number> {
    ...
    try {
      const result = await pool.query(
        `SELECT COUNT(DISTINCT session_id) as count FROM search_history`
      );
